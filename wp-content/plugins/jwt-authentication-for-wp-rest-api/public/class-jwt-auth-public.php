...
//        }
        if (is_wp_error($user)) {
            return new WP_REST_Response(array(
                'message' => 'Helaas, deze combinatie van e-mail en wachtwoord is niet bij ons bekend'
            ), 401);
        }
...
//            'user_display_name' => $user->data->display_name,
    ...
    public function validate_token($output = true)
    {
        ...
        try {
            ...
            if (!isset($token->data->user->id) && !isset($token->data->user->pass)) {
            ...
            if ($token->data->user->pass !== get_userdata( $token->data->user->id )->data->user_pass) {
                return new WP_Error(
                    'jwt_auth_bad_request',
                    __('Token is not valid', 'wp-api-jwt-auth'),
                    array(
                        'status' => 401,
                    )
                );
