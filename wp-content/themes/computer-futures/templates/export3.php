	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-12 col-lg-8">
				...
				<div class="cf-panel">
					...
					<?php if(!isset($_GET['step'])) { ?>
						...
						<div class="cf-panel-b">
							...
							<table id="d-with-search-list" class="display cf-datatable" style="width: 100%;">
								...
								<tbody>
								<?php $reviewIdCookie = isset( $_COOKIE['export3-review-id'] ) ? (int)$_COOKIE['export3-review-id'] : null;
					...
					<?php } else { ?>
						...
						<?php if($_GET['step'] == 2 && isset( $_COOKIE['export3-review-id'] ) && $_COOKIE['export3-review-id']) { ?>
							...
							<div class="cf-panel-b">
								...
								<table id="d-with-search-list" class="display cf-datatable" style="width: 100%;">
									...
									<tbody>
									<?php $usersCookie = (isset( $_COOKIE['export3-user-id'] ) && $_COOKIE['export3-user-id'] && $_COOKIE['export3-user-id'] !== 'null')
										? $_COOKIE['export3-user-id']
										: '';

									$reviewId = $_COOKIE['export3-review-id'];
									...
									foreach ($users as $userId) {
										...
										if($userRole !== 'project-manager') continue;
						...
						<?php if($_GET['step'] == 3 && isset( $_COOKIE['export3-user-id'] ) && $_COOKIE['export3-user-id']) { ?>
							...
							<div class="cf-panel-b">
								<form id="cf-form-user-info">
									<div class="row">
										<div class="col-md-6">
											<?php $compareTypeCookie = (isset( $_COOKIE['export3-compare-type'] ) && $_COOKIE['export3-compare-type'] && $_COOKIE['export3-compare-type'] !== 'null')
												? $_COOKIE['export3-compare-type']
												: ''; ?>
								...
								<table id="d-with-search-list" class="display cf-datatable cf-compare-user" style="width: 100%;">
									...
									<tbody>
									<?php $usersCookie = (isset( $_COOKIE['export3-compare-user-id'] ) && $_COOKIE['export3-compare-user-id'] && $_COOKIE['export3-compare-user-id'] !== 'null')
										? $_COOKIE['export3-compare-user-id']
										: '';
						...
						<?php if($_GET['step'] == 4) { ?>
							...
							<div class="cf-panel-b">
								...
								<?php $periodFrom = isset( $_COOKIE['export3-period-from'] ) ? $_COOKIE['export3-period-from'] : '';
								$periodTo = isset( $_COOKIE['export3-period-to'] ) ? $_COOKIE['export3-period-to'] : '';
								$yearFrom = (isset( $_COOKIE['export3-year-from'] ) && $_COOKIE['export3-year-from']) ? $_COOKIE['export3-year-from'] : date('Y');
								$yearTo = (isset( $_COOKIE['export3-year-to'] ) && $_COOKIE['export3-year-to']) ? $_COOKIE['export3-year-to'] : date('Y'); ?>
						...
						<?php if($_GET['step'] == 5) { ?>
                            ...
                            <div id="cf-export-pdf-data" class="cf-panel-b">
								<?php $reviewId = isset( $_COOKIE['export3-review-id'] ) ? (int)$_COOKIE['export3-review-id'] : '';
								$userId = isset( $_COOKIE['export3-user-id'] ) && $_COOKIE['export3-user-id'] ? $_COOKIE['export3-user-id'] : '';
								$compareType = isset( $_COOKIE['export3-compare-type'] ) && $_COOKIE['export3-compare-type'] ? $_COOKIE['export3-compare-type'] : '';
								$compareUserId = isset( $_COOKIE['export3-compare-user-id'] ) && $_COOKIE['export3-compare-user-id'] ? $_COOKIE['export3-compare-user-id'] : '';

                                $periodFrom = isset( $_COOKIE['export3-period-from'] ) ? (int)$_COOKIE['export3-period-from'] : '';
								$periodTo = isset( $_COOKIE['export3-period-to'] ) ? (int)$_COOKIE['export3-period-to'] : '';
								$yearFrom = isset( $_COOKIE['export3-year-from'] ) ? (int)$_COOKIE['export3-year-from'] : '';
								$yearTo = isset( $_COOKIE['export3-year-to'] ) ? (int)$_COOKIE['export3-year-to'] : '';
			...
			<div class="col-md-12 col-lg-4">
				<?php get_template_part( 'components/component', 'export3-steps' ); ?>
			</div>
		</div>
	</div>
</main>
...
<script>
    jQuery(document).ready(function($) {
        ...
        $("#cf-review-start-s1").on('click', function() {
            ...
            if(selectedReviewId) {
                $.cookie("export3-review-id", selectedReviewId, {path: '/'});
                window.location.href = "<?php echo get_the_permalink(497); ?>/?step=2";
            } else {
        ...
        $("#cf-review-start-s2").on('click', function() {
            ...
            if(selectedUsersId) {
                $.cookie("export3-user-id", selectedUsersId, {path: '/'});
                window.location.href = "<?php echo get_the_permalink(497); ?>/?step=3";
            } else {
        ...
        $("#cf-review-start-s3").on('click', function() {
            ...
            $.cookie("export3-compare-type", selectedCompareType, {path: '/'});
            if(selectedCompareType !== 'users') {
                $.cookie("export3-compare-user-id", '', {path: '/'});
                window.location.href = "<?php echo get_the_permalink(497); ?>/?step=4";
            } else if(selectedUsersId) {
                $.cookie("export3-compare-user-id", selectedUsersId, {path: '/'});
                window.location.href = "<?php echo get_the_permalink(497); ?>/?step=4";
            }
        });
        ...
        $("#cf-review-start-s4").on('click', function() {
            ...
            if(periodFrom && periodTo && yearFrom && yearTo) {
                $.cookie("export3-period-from", periodFrom, {path: '/'});
                $.cookie("export3-period-to", periodTo, {path: '/'});
                $.cookie("export3-year-from", yearFrom, {path: '/'});
                $.cookie("export3-year-to", yearTo, {path: '/'});
                window.location.href = "<?php echo get_the_permalink(497); ?>/?step=5";
            } else {
